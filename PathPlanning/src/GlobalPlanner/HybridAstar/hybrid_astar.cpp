...
int main(int argc, char** argv) {
    ...
    draw_vehicle1(start, 0.0, VC);
    draw_vehicle1(goal, 0.0, VC, "0.4");
    plt::title("Hybrid A*");
    plt::pause(1.0);
    ...
    for (size_t idx = 0; idx < path.x.size(); ++idx) {
        ...
        draw_vehicle1({path.x[idx], path.y[idx], path.yaw[idx]}, steer, VC);
    ...
    draw_vehicle1(start, 0.0, VC, "0.4");
    draw_vehicle1(goal, 0.0, VC);
    plt::title("Hybrid A*");
    plt::axis("equal");
    for (size_t idx = 2; idx < path.x.size(); idx += 4) {
        //utils::draw_vehicle({path.x[idx], path.y[idx], path.yaw[idx]}, 0, VC, "-c", false, false);
        draw_vehicle1({path.x[idx], path.y[idx], path.yaw[idx]}, 0, VC, "-c", false, false);
        plt::pause(0.001);
    }
